...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** Sample counts for 自动/天/月 — multiples of 7. The Host sends the authority. */
    const CHART_POINTS = [14, 21, 28, 35, 42, 49, 56]
    /** Hourly samples step in half-days: multiples of 12, capped at 60. */
    const CHART_HOUR_POINTS = [12, 24, 36, 48, 60]
    const CHART_POINTS_DEFAULT = 28
    const CHART_HOUR_POINTS_DEFAULT = 24
    /**
     * The counts one unit offers. Mirrors the Host's rule exactly, so the panel
     * can render (and validate) before the first reply arrives.
     */
    function chartPointOptions(unit) {
      return unit === 'hour' ? CHART_HOUR_POINTS : CHART_POINTS
    }
    /**
     * Why a unit cannot be served, in the user's terms. Currency pairs are the
     * only case where a unit is structurally missing: ECB publishes one daily
     * reference rate and no intraday series exists for the ordinary pairs.
     */
    function unitUnavailableNote(kind, unit) {
      if (unit !== 'hour') return '该档位暂无数据源'
      if (kind === 'fx') return 'ECB 只发布每日参考价，普通货币对没有小时级数据；可换指数或加密货币标的'
      return '该标的暂无小时级数据源'
    }

    /**
     * A stored count coerced into its unit's list: 28 means nothing for 小时, so
     * switching units must move the count to something the unit can serve.
     */
    function chartPointsFor(unit, stored) {
      const options = chartPointOptions(unit)
      if (options.includes(stored)) return stored
      return unit === 'hour' ? CHART_HOUR_POINTS_DEFAULT : CHART_POINTS_DEFAULT
    }
    ...
    const CSS = [
      ...
      // An option's own colours show through Chrome's popup list; the select's
      // closed state repeats the signal so it never has to be opened to read.
      '.fxm-ctl select.fxm-ok{color:var(--dsw-alias-label-primary);font-weight:600}',
      '.fxm-ctl select.fxm-off{color:var(--dsw-alias-label-secondary)}',
    ...
    function readSettings() {
      ...
      try {
        ...
        return {
          ...
          // A count valid for one unit may be invalid for another; the panel
          // coerces per unit, so accept anything either list allows here.
          chartPoints: CHART_POINTS.includes(parsed && parsed.chartPoints)
            || CHART_HOUR_POINTS.includes(parsed && parsed.chartPoints)
            ? parsed.chartPoints
            : fallback.chartPoints,
        }
      } catch {
        return fallback
      }
    ...
    function DetailPopover(props) {
      ...
      // Never send 28 with 小时: the count is coerced to the chosen unit
      // before it leaves the panel, and the same value is what the select
      // shows, so the control and the request can never disagree.
      const requestPoints = chartPointsFor(settings.chartUnit, settings.chartPoints)
      const trend = useTrend(item.id, settings.chartUnit, requestPoints)
      ...
      // Only trust the reply's list when it describes the unit now selected. While
      // a new request is in flight the previous reply's list is stale, and a
      // <select> whose value is absent from its options silently shows the first
      // one — which is how a coerced 24 came to be displayed as 14.
      const repliedUnit = meta !== null && typeof meta.requestedUnit === 'string' ? meta.requestedUnit : null
      const pointOptions = repliedUnit === settings.chartUnit
        && Array.isArray(meta.pointOptions) && meta.pointOptions.length > 0
        ? meta.pointOptions
        : chartPointOptions(settings.chartUnit)
      ...
      return h('div', { className: 'fxm-panel', style, onMouseDown: (e) => e.stopPropagation() },
        ...
        h('div', { className: 'fxm-chartctl' },
          h('label', { className: 'fxm-ctl' },
            h('span', null, 'X 轴单位'),
            h('select', {
              // `自动` is always a valid choice; only an explicit unit that the
              // symbol could not serve reads as grey.
              className: `fxm-sel ${settings.chartUnit === 'auto' || servedUnit === settings.chartUnit ? 'fxm-ok' : 'fxm-off'}`,
              ...
              onChange: (e) => {
                ...
                settingsStore.set({
                  ...settings,
                  chartUnit: nextUnit,
                  // 12-step hours and 7-step days share no values, so a
                  // carried-over count would land on an invalid option.
                  chartPoints: chartPointsFor(nextUnit, settings.chartPoints),
                })
              },
            }, CHART_UNITS.map((u) => {
              const able = available === null || available.includes(u)
              return h('option', {
                key: u,
                value: u,
                ...
                disabled: !able,
                title: able ? undefined : unitUnavailableNote(item.kind, u),
                style: {
                  // Grey says "no source for this unit"; bold black says "yes".
                  color: able ? 'var(--dsw-alias-label-primary)' : 'var(--dsw-alias-label-secondary)',
                  fontWeight: able ? '600' : '400',
                },
              }, able ? CHART_UNIT_LABELS[u] : `${CHART_UNIT_LABELS[u]}（无数据源）`)
            })),
          ),
          h('label', { className: 'fxm-ctl' },
            h('span', null, '数据组数'),
            h('select', {
              className: 'fxm-sel',
              value: String(requestPoints),
              disabled: countLocked,
              title: countLocked
                ? '「自动」档位下指数显示全天分时线，组数不适用；切到小时 / 天 / 月即可调整'
                : settings.chartUnit === 'hour'
                  ? '小时线按半天步进，可选 12 / 24 / 36 / 48 / 60'
                  : '走势图上画多少个取样点（7 的倍数）',
          ...
          // Says what is actually on the axis — without it, two units that draw
          // the same data (auto and day for a currency pair) look like a bug.
          h('span', { className: 'fxm-tip-x' },
            trend.status === 'loading'
              ? '加载中…'
              : settings.chartUnit === 'auto' && servedUnit !== 'auto'
                // `自动` for a currency pair resolves to daily; name that, or the
                // unchanged axis under 自动 and 天 reads as a broken control.
                ? `当前：自动（${CHART_UNIT_LABELS[servedUnit] || servedUnit}）`
                : `当前：${CHART_UNIT_LABELS[servedUnit] || servedUnit}`),
        ),
        stat === null
        ...
        h('div', { className: 'fxm-note' },
          ...
          degraded
            ...
            : null,
          // A greyed option explains itself on hover; this says it out loud,
          // because 「没有数据源」 otherwise reads as a broken control.
          available !== null && !available.includes(settings.chartUnit)
            ? h('div', { className: 'fxm-err' },
                `「${CHART_UNIT_LABELS[settings.chartUnit]}」不可用：${unitUnavailableNote(item.kind, settings.chartUnit)}`)
            : null,
