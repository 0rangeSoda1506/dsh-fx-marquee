...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function readSettings() {
      const fallback = { symbols: DEFAULT_SYMBOLS.slice(), intervalSec: 20, scheme: 'cn', sparkline: true, fxSource: 'auto', enabled: true }
      try {
    ...
    function DetailPopover(props) {
      ...
      const tone = view.tone === 'fxm-nodata' ? 'fxm-flat' : view.tone
      ...
      return h('div', { className: 'fxm-panel', style, onMouseDown: (e) => e.stopPropagation() },
        ...
        h('div', { className: 'fxm-note' },
          entry && entry.label ? `${entry.label}　${entry.range || ''}` : '',
