...
/** Point counts for 自动/天/月 — every one a multiple of 7. */
export const SERIES_POINT_OPTIONS = [14, 21, 28, 35, 42, 49, 56]
/** Hourly samples step in half-days: multiples of 12, capped at 60. */
export const SERIES_HOUR_POINT_OPTIONS = [12, 24, 36, 48, 60]
/** Used when a request omits or misstates `points` for 自动/天/月. */
export const DEFAULT_SERIES_POINTS = 28
/** The hourly default. */
export const DEFAULT_SERIES_HOUR_POINTS = 24

/**
 * The counts on offer for one unit. Hours step by 12 (half a trading day) and
 * stop at 60; every other unit uses the multiples of 7.
 * @param unit - `auto` | `hour` | `day` | `month`.
 */
export function pointOptionsFor(unit) {
  return unit === 'hour' ? SERIES_HOUR_POINT_OPTIONS : SERIES_POINT_OPTIONS
}

/**
 * Coerce a requested count into its unit's list, falling back to that unit's
 * default — a count valid for 天 (28) has no meaning for 小时.
 * @param unit - `auto` | `hour` | `day` | `month`.
 * @param requested - the requested count.
 */
export function normalizePoints(unit, requested) {
  const options = pointOptionsFor(unit)
  if (options.includes(requested)) return requested
  return unit === 'hour' ? DEFAULT_SERIES_HOUR_POINTS : DEFAULT_SERIES_POINTS
}
...
/**
 * Hourly series are withdrawn for now.
 *
 * The only keyless intraday FX source is eastmoney's kline endpoint, which
 * publishes just `USD/CNH` and `EUR/USD` and is currently refusing every request
 * (`UND_ERR_SOCKET`, on all hosts and numbered subdomains). Offering an hourly
 * option that is grey for nearly every symbol is worse than not offering it, so
 * the unit stays visible but unselectable. Flip this to `true` to bring it back:
 * the loaders, the 12-step counts and their tests all remain in place.
 */
export const HOUR_UNIT_ENABLED = false
...
export function availableUnitsFor(id, kind) {
  const daily = ['auto', 'day', 'month']
  if (!HOUR_UNIT_ENABLED) return daily
  if (kind === 'crypto' || kind === 'index') return ['auto', 'hour', 'day', 'month']
  return EM_FOREX_SECID[id] === undefined ? daily : ['auto', 'hour', 'day', 'month']
}
...
export async function loadSeries(symbols, getText, options = {}) {
  ...
  const points = normalizePoints(requestedUnit, options.points)
  const errors = []
  const now = Date.now()
